...
(function($){$.fn.JSONtip=function(options){var dds=$.extend({},$.fn.JSONtip.defaults,options),tip=$(".JSONtip");if(tip.length==0){$('body').append(makeTip());}
return this.each(function(){var $t=$(this),o=dds,tip=$(".JSONtip"),tipVals;if(dds['inline-parse']==true){o=$.extend({},dds,$t.data());}
if(o['preload']==true&&o['url']){loadJSON($t,o);}
$t.click(function(e){e.preventDefault();e.stopPropagation();$.fn.JSONtip.tipReset(o['tip-class']);tipVals=tipPos($t,o);$(".JSONloading").fadeIn();if(o['auto-position']==true){tipVals=tipPosMove(tipVals,$t,o);}
$.fn.JSONtip.tipPlace(tipVals);$.fn.JSONtip.tipShow(o['fade-in-speed']);if(o['no-close']==true){tip.addClass('noClose');}
if(o['preload']==false&&o['url']&&!$t.data('json')){loadJSON($t,o);$t.ajaxComplete(function(){tipFill($t,o);if(tip.is('.tipTopRight, .tipTopLeft, .tipTopCenter')){tipVals=tipHeightAdjust($t,o,tipVals);$.fn.JSONtip.tipPlace(tipVals);}});}
else{tipFill($t,o);if(tip.is('.tipTopRight, .tipTopLeft, .tipTopCenter')){tipVals=tipHeightAdjust($t,o,tipVals);$.fn.JSONtip.tipPlace(tipVals);}}});tip.click(function(e){e.stopPropagation();});$("body").keyup(function(k){if(k.keyCode==27){$.fn.JSONtip.tipHide(o['fade-out-speed']);}});if(o['body-close']==true){$("body").click(function(){$.fn.JSONtip.tipHide(o['fade-out-speed']);});}
$(".JSONtipClose").click(function(){$.fn.JSONtip.tipHide(o['fade-out-speed']);});});function debug(what){console.log(what);}
function makeTip(){var tip='<div class="JSONtip tipRight "'+dds['tip-class']+'>'+'<span class="JSONtipClose">x</span>'+'<div class="JSONtipArrow">'+'<div class="inArrow"></div>'+'</div>'+'<h4></h4>'+'<p class="JSONtipBody"></p>'+'<img src="/img/loading.gif" alt="loading" class="JSONloading" />'+'</div>';return tip;}
function loadJSON(what,o){var url=o['url'],code=what.data('jsondata')?what.data('jsondata'):o['jsonData'];if(what.data('json')){return;}
if(dds.preload==false){$(".JSONloading").fadeIn('fast');}
$.getJSON(url,{code:code},function(js){what.data('json',js);});}
function tipFill(what,op){var td=what.data(),tipH=$(".JSONtip h4"),tipS=$(".JSONtip .JSONtipBody");if(op['url']){tipS.html(what.data('json').tipbody);if(td.json.tiphead){tipH.html(what.data('json').tiphead);}}
else if(td.tipbody){tipS.html(td.tipbody);if(td.tiphead){tipH.html(td.tiphead);}}
else{tipS.html(what.title);}
showHead();$(".JSONloading").fadeOut();}
function showHead(){var yes='block',tipH=$(".JSONtip h4");if(tipH.html()==''){yes='none';}
tipH.css({'display':yes});}
function tipPos(what,op){var eOff=what.offset(),eLeft=eOff.left,eTop=eOff.top,eHeight=what.outerHeight(),eWidth=what.outerWidth(),defX=op['x-offset'],defY=op['y-offset'],xTip=(eHeight+eLeft)+defX,yTip=(eTop)-defY,userTip=op['tip-position'].toLowerCase(),tipVals={},tipDms=$(".JSONtip"),tipClass='';switch(userTip){case'left':xTip=(eLeft-tipDms.width())-defX;tipClass='tipLeft';break;case'topleft':xTip=(eLeft)-defX;yTip=(eTop-tipDms.height())-defY;tipClass='tipTopLeft';break;case'topright':xTip=(eLeft-tipDms.width())+eWidth-defX;yTip=(eTop-tipDms.height())-defY;tipClass='tipTopRight';break;case'topcenter':xTip=(eLeft+eWidth/2)-(tipDms.width()/2)-defX;yTip=(eTop-tipDms.height())-defY;tipClass='tipTopCenter';break;case'bottomleft':xTip=(eLeft)-defX;yTip=(eTop+eHeight)+defY;tipClass='tipBottomLeft';break;case'bottomright':xTip=(eLeft-tipDms.width())+eWidth-defX;yTip=(eTop+eHeight)+defY;tipClass='tipBottomRight';break;case'bottomcenter':xTip=(eLeft+(eWidth/2))-(tipDms.width()/2)-defX;yTip=(eTop+eHeight)+defY;tipClass='tipBottomCenter';break
default:xTip=(eLeft+eWidth)+defX;tipClass='tipRight';break;}
return tipVals={left:xTip,top:yTip,c:tipClass};}
function tipPosMove(vals,what,op){var wL=window.innerWidth,eOff=what.offset(),defX=op['x-offset'],tipDms=$(".JSONtip"),x=vals.left,cl=vals.c,valsV={};if(vals.x<0){switch(vals.c){case'tipBottomCenter':x=0+defX;break;case'tipTopCenter':x=0+defX;break;case'tipTopRight':x=eOff.left+defX;cl='tipTopLeft';break;case'tipBottomRight':x=eOff.left+defX;cl='tipBottomLeft';break;default:x=eOff.left+what.width()+defX;cl='tipRight';break;}}
else if((vals.x+tipDms.width())>wL){switch(vals.c){case'tipBottomCenter':x=wL-what.width()-defX;break;case'tipTopCenter':x=wL-what.width()-defX;break;case'tipTopLeft':xl=(eOff.left-tipDms.width()+what.width())-defX;cl='tipTopRight';break;case'tipBottomLeft':x=(eOff.left-tipDms.width()+what.width())-defX;cl='tipBottomRight';break;default:x=eOff.left-what.width()+defX;cl='tipLeft';break;}}
vals.left=x;vals.c=cl;return vals;}
function tipHeightAdjust(what,op,vals){var tip=$(".JSONtip"),tipH=tip.outerHeight(),tipT=tip.offset(),eOff=what.offset();vals.top=eOff.top-tipH-op['y-offset'];return vals;}}
$.fn.JSONtip.defaults={'fade-in-speed':500,'fade-out-speed':200,'x-offset':22,'y-offset':22,'tip-position':'right','auto-position':true,'body-close':true,'no-close':false,'tip-class':'','url':null,'jsondata':null,'inline-parse':false,'preload':true};$.fn.JSONtip.tipShow=function(speed){var s=speed?speed:'500'
$(".JSONtip").fadeIn(s);}
$.fn.JSONtip.tipHide=function(speed){var s=speed?speed:'300'
$(".JSONtip").fadeOut(s);}
$.fn.JSONtip.tipReset=function(classes){$(".JSONtip").removeClass().addClass('JSONtip tipRight '+classes);$(".JSONtip h4").html('');$(".JSONtip .JSONtipBody").html('');}
$.fn.JSONtip.tipPlace=function(where){var here={left:0,top:0,zIndex:9000,display:'block',c:'tipRight'}
here=$.extend({},here,where);$(".JSONtip").css(here).removeClass('tipRight').addClass(here.c);}})(jQuery);;(function($){var $scrollTo=$.scrollTo=function(target,duration,settings){$(window).scrollTo(target,duration,settings);};$scrollTo.defaults={axis:'xy',duration:parseFloat($.fn.jquery)>=1.3?0:1};$scrollTo.window=function(scope){return $(window)._scrollable();};$.fn._scrollable=function(){return this.map(function(){var elem=this,isWin=!elem.nodeName||$.inArray(elem.nodeName.toLowerCase(),['iframe','#document','html','body'])!=-1;if(!isWin)
